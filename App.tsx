...
const STORAGE_KEY = 'ARSHIF_PRO_V10_FILES';
const AUDIT_KEY = 'ARSHIF_PRO_V10_AUDIT';
const INTEG_KEY = 'ARSHIF_PRO_V10_TELEGRAM';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (savedFiles) {
        try { setFiles(JSON.parse(savedFiles)); } catch(e) { console.error("Data load error", e); }
    }
    if (savedAudit) {
        try { setAuditLogs(JSON.parse(savedAudit)); } catch(e) {}
    }
    if (savedInteg) {
        try { setIntegrations(JSON.parse(savedInteg)); } catch(e) {}
    }
  }, []);
  ...
  useEffect(() => {
    const runAnalysis = async () => {
      ...
      try {
        let analysis;
        // التعامل مع الملفات بناءً على توفر الملف الأصلي
        if (pending.originalFile) {
          ...
          // استرجاع المحتوى النصي إذا كان مخزناً
        ...
        setAuditLogs(prev => [{ 
          ...
          details: `تم الانتهاء من التحليل الذكي للوثيقة: ${pending.name}`, 
          user: 'Gemini AI', 
        ...
        if (data.ok && data.result.length > 0) {
          for (const upd of data.result) {
            ...
            if (upd.message && String(upd.message.chat.id) === String(adminChatId) && upd.message.text) {
              ...
              await sendToTelegram("🤖 <b>جاري معالجة طلبك...</b>");
              
              // سياق محدود لتوفير التوكنات
              const ctx = filesRef.current.slice(0, 15).map(f => 
                `ID: ${f.isoMetadata?.recordId} | File: ${f.name} | Summary: ${f.isoMetadata?.executiveSummary}`
              ).join('\n');
  ...
  // معالجة اختيار المجلد (Folder Select)
  const handleSelectFolder = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (let i = 0; i < sel.length; i++) {
      const f = sel[i];
      // تجاهل الملفات المخفية أو النظامية
      ...
      newRecords.push({
        ...
        originalFile: f, 
        isProcessing: true, // وضع الملف في طابور المعالجة
        isoMetadata: {
          recordId: `REC-${Date.now().toString().slice(-6)}-${i}`, 
          title: f.name, 
          description: "في انتظار التحليل...", 
          documentType: DocumentType.OTHER, 
          entity: "الأرشيف العام", 
  ...
  const handleChat = async () => {
    ...
    try {
      ...
      for await (const chunk of stream) {
        fullText += chunk;
        setMainChatMessages(p => p.map(m => m.id === botId ? { ...m, text: fullText } : m));
      }
    } catch { 
      setMainChatMessages(p => p.map(m => m.id === botId ? { ...m, text: "عذراً، حدث خطأ أثناء الاتصال بالوكيل." } : m));
    }
  ...
  const handleVerifyTelegram = async () => {
    ...
    if (!botToken || !adminChatId) return alert("يرجى تعبئة جميع الحقول.");
    ...
    try {
      ...
      if (data.ok) {
        ...
        await fetch(`https://api.telegram.org/bot${botToken}/sendMessage`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ chat_id: adminChatId, text: "✅ <b>تم تفعيل أرشيف PRO بنجاح.</b>", parse_mode: 'HTML' })
        });
        setIntegrations(p => ({ ...p, telegram: { ...p.telegram, connected: true } }));
        alert(`تم الربط بنجاح مع البوت: ${data.result.first_name}`);
      } else {
        alert("فشل التحقق من التوكن.");
      }
    } catch (e) {
      alert("خطأ في الاتصال بخوادم تليجرام.");
    } finally {
  ...
  const handleReset = () => {
    if (confirm("⚠️ تحذير: سيتم حذف كافة البيانات والملفات. هل أنت متأكد؟")) {
      setFiles([]);
      setAuditLogs([]);
      setIntegrations({
      ...
      <aside className="w-80 bg-slate-900 text-slate-400 flex flex-col fixed h-full z-20 shadow-2xl border-l border-slate-800">
        <div className="p-8">
          <div className="flex items-center gap-4 mb-12">
            ...
            <div>
              ...
              <span className="text-[10px] text-indigo-400 font-bold uppercase tracking-widest">System V10</span>
            </div>
          </div>
        ...
        <div className="mt-auto p-8 border-t border-slate-800">
           <div className="flex items-center gap-3">
              ...
              <span className="text-xs font-bold text-slate-500 uppercase tracking-widest">{integrations.telegram.connected ? 'Telegram Active' : 'Offline Mode'}</span>
           </div>
        </div>
      </aside>
          ...
          <div className="max-w-6xl mx-auto space-y-8 animate-saas">
            <header className="flex justify-between items-center bg-white p-8 rounded-[2.5rem] border shadow-sm">
              <div>
                ...
                <p className="text-slate-400 font-bold mt-1">نظام إدارة الأرشيف والذكاء الاصطناعي.</p>
              </div>
              <div className="flex gap-4">
                 <div className="px-6 py-3 bg-indigo-50 text-indigo-600 rounded-2xl font-bold flex items-center gap-2 border border-indigo-100 shadow-sm">
                    <Zap size={18} className="animate-pulse" /> Gemini Engine Ready
                 </div>
              </div>
            </header>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              <div className="lg:col-span-2 space-y-8">
                <div className="grid grid-cols-2 gap-6">
                  <div className="bg-white p-8 rounded-[2rem] border shadow-sm flex items-center justify-between">
                    <div><p className="text-xs font-black text-slate-400 uppercase mb-2 tracking-widest">إجمالي الملفات</p><h3 className="text-4xl font-black text-slate-800">{files.length}</h3></div>
                    <div className="bg-indigo-50 p-5 rounded-2xl text-indigo-600"><Database size={28} /></div>
                  </div>
                  <div className="bg-white p-8 rounded-[2rem] border shadow-sm flex items-center justify-between">
                    <div><p className="text-xs font-black text-slate-400 uppercase mb-2 tracking-widest">تليجرام</p><h3 className="text-4xl font-black text-blue-600">{integrations.telegram.stats.messagesSent}</h3></div>
                ...
                <div className="bg-slate-900 rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col h-[520px] relative">
                   <div className="p-6 border-b border-white/10 flex items-center gap-3 bg-slate-800/50 text-white">
                      ...
                      <h3 className="font-black text-sm">المساعد الذكي (Chat Agent)</h3>
                   </div>
                   ...
                   <div className="p-4 bg-slate-800 border-t border-white/10">
                      <div className="flex gap-2 bg-slate-900 p-2 rounded-xl border border-white/5 shadow-inner">
                         <input type="text" className="flex-1 bg-transparent border-none outline-none text-white px-3 py-2 text-sm font-bold" placeholder="اسأل عن أي مستند..." value={mainChatInput} onChange={e => setChatInput(e.target.value)} onKeyPress={e => e.key === 'Enter' && handleChat()} />
              ...
              <div className="bg-white p-8 rounded-[2.5rem] border shadow-sm flex flex-col">
                <h3 className="text-lg font-black text-slate-800 mb-6 flex items-center gap-2 tracking-tighter"><History size={20} className="text-indigo-600" /> آخر الأنشطة</h3>
          ...
          <div className="max-w-7xl mx-auto space-y-8 animate-saas">
            <header className="flex justify-between items-center bg-white p-8 rounded-[2.5rem] border shadow-sm">
              <div>
                ...
                <p className="text-slate-400 font-bold mt-1">رفع وتصنيف المجلدات والملفات.</p>
              </div>
              <div className="flex gap-4">
                <div className="relative w-80 shadow-sm rounded-2xl overflow-hidden">
                  <Search className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400" size={18} />
                  <input className="w-full pr-12 pl-4 py-4 bg-slate-50 border-2 border-transparent focus:border-indigo-500 outline-none font-bold text-sm" placeholder="بحث في الأرشيف..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
                </div>
                {/* Input معدل لرفع المجلدات */}
                <input 
                  type="file" 
                  ref={folderInputRef} 
                  className="hidden" 
            ...
            {isScanning && (
              <div className="bg-indigo-600 text-white p-12 rounded-[2.5rem] shadow-2xl flex flex-col items-center gap-6 animate-in fade-in zoom-in">
                ...
                <h3 className="text-2xl font-black tracking-tight">جاري مسح المجلد ورفع الملفات... {scanProgress}%</h3>
                <p className="font-bold opacity-80">سيتم بدء التحليل الذكي تلقائياً لكل ملف.</p>
              </div>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
              {files.filter(f => f.name.toLowerCase().includes(searchQuery.toLowerCase())).map(file => (
                <div key={file.id} onClick={() => setSelectedFileId(file.id)} className="bg-white p-8 rounded-[2.5rem] border shadow-sm hover:shadow-2xl transition-all cursor-pointer relative group overflow-hidden">
                  {file.isProcessing && (
                    <div className="absolute top-6 left-6 animate-pulse bg-indigo-50 text-indigo-600 px-3 py-1 rounded-full text-[10px] font-black border border-indigo-100 flex items-center gap-1 shadow-sm z-20">
              ...
              {files.length === 0 && !isScanning && (
                <div className="col-span-full py-40 flex flex-col items-center justify-center bg-white rounded-[3rem] border-2 border-dashed border-slate-200 opacity-60">
                   ...
                   <p className="text-slate-500 font-bold mt-2">اختر "رفع مجلد كامل" للبدء.</p>
                </div>
              )}
            </div>
          </div>
                  ...
                  <div className="space-y-10 animate-in fade-in">
                    <div className="flex items-center justify-between">
                       ...
                       <div className={`px-4 py-1 rounded-full text-[10px] font-black border ${integrations.telegram.connected ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-slate-50 text-slate-400 border-slate-100'}`}>
                          {integrations.telegram.connected ? 'System Online' : 'System Offline'}
                       </div>
                    </div>
                    <div className="space-y-6 max-w-lg">
                      ...
                      <button onClick={handleVerifyTelegram} disabled={isVerifying} className="bg-slate-900 text-white w-full p-5 rounded-2xl font-black flex items-center justify-center gap-3 hover:bg-slate-800 transition-all shadow-xl disabled:opacity-50">
                        {isVerifying ? <Loader2 className="animate-spin" /> : <ShieldCheck />} حفظ والتحقق من الاتصال
                      </button>
                    </div>
                  </div>
                )}
           ...
           <div className="bg-white w-full max-w-4xl rounded-[3rem] shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
              ...
              <div className="flex-1 overflow-y-auto p-12 space-y-10 custom-scroll">
                 <div className="bg-indigo-50 p-8 rounded-[2.5rem] border border-indigo-100 shadow-inner relative overflow-hidden">
                    ...
                    <h4 className="font-black text-indigo-600 mb-4 flex items-center gap-2 uppercase tracking-tighter text-xs font-bold"><Sparkles size={18} /> التحليل الذكي (Gemini Analysis)</h4>
                    <p className="text-slate-800 leading-9 text-sm font-bold text-justify whitespace-pre-wrap">{files.find(f => f.id === selectedFileId)?.isoMetadata?.executiveSummary || "جاري التحليل..."}</p>
                 </div>
                 <div className="grid grid-cols-2 gap-6">
                    {[
